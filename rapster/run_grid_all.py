# python3 run_grid.py --npoints 2 --ncpu 4 --outdir /path/to/outdir
# python3 run_grid_all.py --npoints 2 --ncpu 4 --outdir /mnt/n fs/DataStudents/ilaria.usai/Rapster270/Rapster/rapster/
import argparse
import itertools
import subprocess
import numpy as np
...
def build_grid(n, n_z):
    ...
    # sample linearly in lookback time [11, 13.5] Gyr, then convert to redshift
...
def run_sim(args):
    Z, Mcl, r, R_pc, z_form, base_outdir, dry_run = args
    ...
    return tag, Z, Mcl, r, R_pc, z_form, result.returncode


def merge_outputs(results, base_outdir):
    """Concatenate all per-simulation files into two global files,
    adding columns: ID, Z_grid, Mcl_grid, r_grid, R_grid, z_form_grid."""
...
def main():
    ...
    parser.add_argument("--nz",      type=int, default=3,  help="Points for z_form (sampled linearly in lookback time 11-13.5 Gyr)")
    ...
    jobs = [(Z, Mcl, r, R, z_form, args.outdir, args.dry_run) for Z, Mcl, r, R, z_form in grid]
    results = []
    done    = 0
    failed  = []
